...
PAIRS = [
    ...
    ("参赛挑法 ", "参赛的选取策略 "),
    ("（最新 as_of ", "（最新一期 "),
    # 字段名/枚举值当成人话印出来（2026-09-05 第三轮）
    ("只用跑满部分重算的结果见 horizon_completeness。",
     "只用跑满部分重算的结果见「持有期完整度」（horizon_completeness）。"),
    ("：seed 期打的是人工在 2026-07 撰写的主题词典，discovered 期的主题是该期自己从当周研报里发现并命名的。",
     "：人工词典（seed）那几期打的是 2026-07 人工撰写的主题词典，"
     "自己发现（discovered）那几期的主题是该期自己从当周研报里发现并命名的。"),
    ("两类不要合成一个胜率读，分列见 theme_provenance。",
     "两类不要合成一个胜率读，分列见「排序力」表里的主题来源标注（theme_provenance）。"),
    ("verdict_stable_across_depths 为 false 的组合",
     "判定不随剪切深度稳定（verdict_stable_across_depths=false）的组合"),
    ("（mixed ", "（混合 "), ("、seed ", "、人工词典 "), ("、discovered ", "、自己发现 "),
    ("（seed ", "（人工词典 "), ("（discovered ", "（自己发现 "),
    # 语料 → 研报
