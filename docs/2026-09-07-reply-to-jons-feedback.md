...
**选股能力研究（30 天持有，无模型，`bt-real-20260902-e1622692c7`，六期全为补跑；2026-09-07 净值载入后重跑）**：ev_rank 胜率 72% / 均值 +2.63%（18 个标的×期），mom_21 67% / +1.43%（60），buy_all 60% / +1.02%（174），spread 67% / +0.90%（24），omega_loose 68% / +0.76%（31），random_pick 58% / +0.63%（26），left_tail 51% / +0.35%（47），calib 65% / +0.22%（26），generated_carl_constraint 40% / −0.29%（50），generated_ai_native 38% / −0.33%（50），omega_strict 60% / −0.35%（20）。两个「来源限定」组合第一次有数：它们选的基金里有净值序列的那部分被计入，其余仍记为「缺前后两个净值点」（buy_all 473 条想法里 299 条如此、来源限定各约 240 条）——这是数据缺口的如实记录，不是 0 填充。净值载入前的上一版数字（ev_rank 87% / +3.75%、random_pick 90% / +2.13% 等）已被取代。前推检验：剔除事后设计的组合后，跟随领先者 +0.50%/期 vs 随机挑一条 +0.65%，相对全量基准的优势 +0.02%/期，与上一版结论一致：挑组合这个动作没有优势。
...
### 模拟组合按新判决重建（PM 决定，2026-09-07）

重跑用的是不建仓模式，十个模拟组合里的仓位仍来自被取代的旧运行，持仓页与方法页两套历史对不上。按 PM 选择「归档旧组合、按新运行重建」执行：`ideagen restate-books --confirm`。

- 旧组合整体改名归档为 `old:sel-<策略>@20260907T092542`（939 个仓位、订单、成交、净值、盯市记录一行不删），落在所有「组合」读取路径（盯市循环、面板、业绩页）之外；旧批次标 `status=superseded`，按期读批次的入口都跳过它。
- 新 `sel-<策略>` 组合按六期各自最新运行的判决，用模拟运行同一套规则按当期日历回放到 09-04（与正式回测同一段代码），组合说明里写明重建时间与来源；从周三起周跑照常往这些组合里建仓。
- 结果：12 个组合（含期望值排序、一月动量），AI 端到端 / 两个来源限定 / 一月动量为空（原因同正式回测）；其余组合各期都有仓位，持仓页、各期页、业绩页现在读的都是新链路的账。
- 口径提醒：这些仓位是今天按已存在的 K 线回放出来的，不是当时向前下的单；业绩页「按时运行」子集在下一期之前为空。库文件重建前备份：`data/ideagen.db.bak-before-restate-20260907`。

### 重建后暴露的两处问题（PM 2026-09-07 指出，已修）

1. **业绩页打开全是灰的**：默认子集写死为「按时运行」，重建后所有仓位都是补跑分类，这个子集为空。改为默认子集由数据决定（`perf_index.modes[paper].default_subset`，没有按时仓位就默认「全部」并写明原因）。
2. **候选池一堆 LU/ISIN 基金，组合里没几个**：2026-09-02 的 79 只候选里 61 只是 Olive 货架基金，本机 `navs`/`shelf_navs` 净值表为空（211 只基金 `priceable=0`），它们在建仓时被「当日无价剔除」，AI 端到端等策略选的几乎全是这类，整本为空——筛选 C 一直在比较一堆根本无法盯市的候选。改法：候选池给每只候选打 `markable` 标记（表里带「无净值」标签，池子顶部写明只数），**筛选 C 只对可盯市的候选做选取**，剔除数写进运行记录（journal 步 `selectors:markable`）；候选池本身仍保存全部候选。要让基金真正回到组合，需要 Olive 净值腿提供历史净值，这是数据问题不是选取逻辑问题。

**Jon 问「为啥 AI 端到端选取没数」**：核实结果比上面更极端——四种生成方法里 ai_native、chain、carl_constraint 提出的标的**全部**是货架基金（09-02 期分别 48、41、46 只，可盯市 0 只），只有 gap 提出 ETF（47 只里 18 只可盯市）。所以 AI 端到端每期选的 10 只全是基金，建仓时全部剔除；两个「来源限定」组合（只保留某方法提出的标的）同理；一月动量选 0 是候选没有行情。处理：对六期用 `ideagen reselect` 重做这四个组合的筛选 C（只对可盯市候选、给候选配上行情），AI 端到端与一月动量各期选出 10 只并重建组合（正式回测：AI 端到端 +3.43%、一月动量 +2.37%）；两个「来源限定」组合仍为空，状态里写明原因（该方法提出的标的全部无净值序列）。这意味着目前模拟账户里的全部仓位实际来自 gap 方法提出的 ETF，其余三种方法的想法在净值数据到位前进不了组合。

**为什么之前拉不到基金净值（PM 追问）**：Olive 的 `shelf_performance` 其实返回每只基金约每月 20 个净值点（接近逐日），但每个点只标注月份不标注日期，仓库里的代码因此只取每月最新一个点，`navs` 表里每只基金只有一条（2026-08-31 的快照净值）。本轮处理：
- 新增 `sources/olive_nav.py`：按「已完整月份从月末工作日倒推、当月从月初正推」推定日期，多于工作日的月份拒绝；所有推定行带 `src=olive:perf:inferred-d`，绝不覆盖 Olive 自己标日期的净值；有 ≥20 个点的基金标为可盯市。CLI `olive-nav-history --dir`。
- 建仓前的定价判断（`booking._priced_only`）加上基金净值路径；基金想法的参考价与 σ（止损/止盈）改由净值序列计算（`olive.horizon_sigma`），与 ETF 同一条规则。
- 历史净值通过 Claude 会话的 Olive 连接器逐只拉取（211 个 key 里 151 个是真实产品代码，其余是本地夹具/ISIN 键，Olive 不认）；本机自己的 Olive OAuth 已过期（每小时日同步报 invalid_grant），每天的新净值需要重新授权一次 `ideagen olive-auth`。
- 净值载入结果：151 个真实产品代码里 121 个有序列、30 个空；81 只载入（22,217 行，最早 2013 年），其中 40 只近 90 天内周频以上、标为可盯市（月度净值的基金载入但不标：模拟引擎要求成交时净值不超过 3 天旧，月度基金几乎成交不了）。
- **快照净值与历史序列基准不一致**：53 只基金的货架快照 NAV 与 `shelf_performance` 序列相差数倍到数十倍（如 L03244 快照 421.67、序列 ~14,750，是不同份额/单位）。第一次重建时这条快照点留在序列里，成了 −97% 的尖刺，三只基金在三本组合里被止损各亏 24 万、全部组合转负。已加基准一致性校验（偏离 25% 以上的快照点按不同基准处理，用序列值替换并记录冲突）后重建。
- 六期全部策略重做筛选 C（可盯市候选从每期 16–19 只升到 26–45 只，含基金）、重建 12 本组合、重跑正式回测（`bt-formal-20260902-2265dcde0c`）：ai_native +2.05%、ev_rank +2.13%、buy_all +1.39%、omega_strict +1.31%、omega_loose +1.21%、spread +1.15%、left_tail +0.67%、random_pick +0.63%、calib +0.33%、generated_carl_constraint −0.61%、generated_ai_native −1.02%；12 个组合六期都有仓位，基金想法第一次真正进入组合。推定日期的仓位口径已写进组合说明与 disclosures。
...
## 2026-09-08 追加：面板两页重做（PM 走查后）

- **分工**：业绩页答「发生了什么」（净值、回撤、跟基准的差，按时/回补子集），不动；证据页只答「这算不算本事」——同样的收益是运气、是 beta，还是选股/排序真有信息。
- **证据页**：一句判决 + 四颗数据截至芯片（周跑 / 组合重建 / 正式回测 / 净值最新日，全部从数据取、可点）→ 假设检验看板（每条预注册假设一张卡：判定 + 一张图 + 一行数，整卡点开右侧抽屉，抽屉里是估计量、对照组、置信区间、详细检验与结论修订记录）→ 假设检验汇总（每层可点）→ 样本累积进度 → 初版回测（30 日固定持有）灰卡 → 数据来源。页面上不再有折叠块和收益表；主线只用纸面组合 + 正式回测这一套数字。「相对等权全池基准的超额」不单独成卡：纸面组合按正式回测同一引擎重建，第一期实时运行前两图逐条相等，并进第一张卡的抽屉作回放对照。
- **口吻**：全部改成投研写法：显著 / 效应缩水 / 不显著 / 样本不足；夏普 95% 置信区间（Lo 2002）、DSR、walk-forward、PBO、MDE、等权全池基准。后端产出的说明文本（`sep.note`、`fd.message`、预注册原句等）仍是旧口吻，另一轮改。
- **概览**：六个环节格、时间线每格、四个数字卡、状态芯片、账面图、告警行全部是入口；06「给自己记分」去业绩页。
- **导览**：从一份改成每页一份，第一次打开某页自动带一遍，按 ? 重看当前页。
- **选股能力研究**（30 日持有）在净值载入后重跑：ev_rank 72% / +2.63%，两个来源限定组合第一次有数（−0.29% / −0.33%），跟随领先者 +0.50%/期 vs 随机 +0.65%，仍无择优优势。
