...
Theme allocation follows the day's scores:
  ENERGY-SUPPLY   TIS 73.3, M 1.4  — highest impact and completely unpriced -> 5
  GEOPOLITICS     TIS 52.1, C 77   — impact but crowded -> 4, all wait-for-pullback
  EARNINGS-QUALITY TIS 52.0, D 100, C 79 — loudest theme, also crowded -> 4, halved size
  COMMODITY-CYCLE TIS 51.5, C 36   — clean setup -> 3
...
SPEC: list[tuple] = [
    ...
    ("XLE", "ENERGY-SUPPLY", "EN-1M-PRODUCER", "1个月", "可执行",
     ...
     "ENERGY-SUPPLY 当日 TIS 73.3 为全场最高，N=92（新事实广度与因果深度都满），"
     "但 M=1.4——预注册指标 XLE 在三日窗口内几乎没动，属于典型的高冲击、未定价。"
     "炼油资产与运输风险同时出现在市场日报与投行研报两层来源，Williams Companies "
     "二季度电话会给出中游吞吐的一手证据。XLE 距 52 周高点 -9.0%，60 日动量仅在"
     "38 百分位，说明价格还没把供给风险当成基准情形。",
    ...
    ("ITA", "GEOPOLITICS", "GEO-6M-DEFENCE", "6个月", "等待回踩",
     ...
     "GEOPOLITICS 的 B=60.2 是当日第二高，说明冲突路径本身仍有真实分歧，"
     "这通常是重定价的前提。但 C=77（偏拥挤）：ITA 距 52 周高点仅 -0.5%、"
    ...
    ("QUAL", "EARNINGS-QUALITY", "EQ-6M-QUALITY", "6个月", "等待回踩",
     ...
     "EARNINGS-QUALITY 的 D=100（166 条独立条目、159 个来源），是全场最被讨论的"
     "一条线，本窗口迪士尼、Unity、闪迪、药明康德等多份一手财报都落在这里。"
     "但 C=79.3：QUAL 动量 82 百分位、距高点 -1.0%。因此保留质量因子作为核心，"
     "动作定为等待回踩而不是追高。",
    ...
    ("COPX", "COMMODITY-CYCLE", "CM-6M-METALS", "6个月", "等待回踩",
     ...
     "COMMODITY-CYCLE 的 C=35.6 是主要主题里最低的，说明这条线还没变成共识。"
    ...
    ("TLT", "POLICY-PATH", "PP-6M-DURATION", "6个月", "仅观察",
     ...
     "TERM-PREMIUM 主题当日 M=72.1（已有确认）——期限溢价上行已经被交易。"
    ...
    ("SMH", "AI-CAPEX", "AI-6M-SEMI", "6个月", "等待回踩",
     ...
     "AI-CAPEX 的 C=12.0——本批次所有主题里最不拥挤，而 N=68 仍然不低。"
    ...
    ("MCHI", "CHINA-POLICY", "CN-6M-EQUITY", "6个月", "等待回踩",
     ...
     "CHINA-POLICY 的 TIS 只有 16.5（背景级），但 M=84.2——"
     "价格已经充分反映，而语料层面几乎没有新事实（5 条、5 个来源）。"
     "这正是框架 §9.3「交易成熟」应当检查拥挤与反转的情形，"
     "因此仓位压到 0.8% 并要求回踩。MiniMax 纳入沪港通（company:99318）是唯一新增事实。",
    ...
    ("PAVE", "AI-POWER", "AP-6M-GRID", "6个月", "可执行",
     ...
     "AI-POWER 当日只有 2 条语料（未通过入选门槛），说明这条线在本窗口"
     "完全不被讨论——但瓶颈本身是物理事实，不因讨论热度而消失。"
...
def main() -> None:
    ...
    batch = {
        ...
        "note": "能源供给风险是当日唯一高冲击且完全未定价的主线；国防与盈利质量"
                "冲击不低但已偏拥挤，一律降级为等待回踩并减半仓位。",
        "macro_narrative":
            "当日 16 个词典主题里，只有能源供给与运输风险同时满足高冲击与低入价："
            "TIS 73.3 全场第一，N=92（新事实广度与因果深度都接近满分），"
            "而预注册指标 XLE 在三日窗口内几乎没动，M=1.4。"
            "与之对照，讨论最密集的盈利兑现与质量分化 D=100（166 条独立条目），"
            "但拥挤度 C=79.3、动量普遍在 80 百分位以上——它是最被谈论的主题，"
            "也是最被交易过的主题。地缘冲突的 B=60.2 说明路径分歧仍在，"
            "但 ITA 距 52 周高点只有 0.5%。"
            "因此本批次的结构是：能源与商品补库两条低入价线承担主要仓位并允许直接执行；"
            "国防、盈利质量、医疗、网络安全这些已经跑过的线一律改为等待回踩、仓位减半；"
            "AI 硬件因为 C=12（全场最不拥挤）而在低仓位下保留完整表达；"
            "中国主题 M=84.2 已属交易成熟，只留 1.5% 的观察性敞口。"
            "另设 4.5% 的短端 carry 仓位，把 14 条等待触发的想法在触发前的"
            "机会成本显性化。",
