...
**概览** —— cockpit，打开先看这个
10 个高层指标（跑赢 SPY 天数 / 每日组合平均收益 / 最好最差的一天 / 想法等权收益 /
排序 ρ / 概率技能分 …）· **收益日历**（每格一天，点进去跳到那天的组合）·
每日组合收益柱状图（灰点是同期 SPY，柱子低于灰点＝跑输）· 每天在赌什么（一行一天）



**日报** —— 引擎当天读到了什么、得出了什么
宏观主线叙述 · 语料底稿（窗口条目、分层、分线）· 主题表（TIS / D A B N / 入价 M /
拥挤 C / 主要争议）· Theme Map（主题 └ 传导主线 └ 资产信号 └ 想法）·
当日 40 条（表格 + 按主题分组的逐条展开）· 来源底稿引用

**组合** —— 买了什么、什么时候出、so far 如何
两个组合的关键数字 · 净值曲线 · 逐条持仓（成本价 / 现价 / 投入 / 盈亏 / 止损 /
止盈 / 到期日 / 状态）· 未成交订单 · 想法层面能力（排序 ρ、Brier 校准）· 分档归因 · 当日告警

不确定的名词旁边都有 `ⓘ`，鼠标悬停或键盘聚焦就出解释；侧边栏「名词表」是完整的 21 条
（TIS / D / A / B / N / M / C / hurdle / 赔率 / 评级 / 幅度校验 / Brier / 技能分 / 排序 ρ …）。

`←` `→` 换日期，`0` `1` `2` 换视图；侧边栏日期轨上的圆点表示那天有没有想法批次。
URL hash 会记住位置（`#book/2026-08-07`），可以直接分享到某一天某一屏。
