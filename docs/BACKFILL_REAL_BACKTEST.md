...
## 补跑给得出什么、给不出什么（2026-09-04 实测确认）

**给得出**:回测。`backtest_positions` 直接用「当期候选池 + 当期之后的真实收盘」算
每条想法 30 天后的结果,不经过建仓,所以历史期次能立刻得到可比的胜率与均值。

**给不出**:历史持仓。补跑期的订单 `placed_d` 是**今天**,不是当期日期——
`first_fillable` 按批次的 `generated_at` 而非 `as_of` 定成交日,故意如此:
用已经印出来的 K 线给事后生成的想法成交就是作弊。代价是补跑期的模拟账
从今天才开始建仓,面板上「08-12 期」的仓位其实是今天买的。

所以:**回测看历史,模拟账看当下**。两者都真,但不是同一件事,别混着讲。

**第三个局限:补跑期的仓位挤在同一天。** 每期只能用 25% 资本(tranche 约束),
真实运行是四期滚动、老仓到期释放现金。补跑把 5 期的订单全压在今天下单,
到期日也全在同一个月后 —— 现金一次性被要求 125%,必然有一批订单因资金不足
成交不了。这不是 bug,是"把四周压成一天"的必然结果。看模拟账时要知道:
补跑期的建仓规模不代表系统正常运行时的仓位节奏。

（顺带修掉的:未来交易日还没印出来时,订单有效期会塌缩成一天,84 张单
本会在成交前过期——见 commit 9e4dc19。）
