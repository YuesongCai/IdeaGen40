...
**它不是补跑独有的，等补跑过去也不会自己好**（2026-09-05 量过）：

  正常周跑（成文→成交滞后 ≤2 天）  335 笔  价差中位 0.644%  p90 1.842%  跑出带宽 3 笔（0.9%）
  补跑        （滞后 >2 天）        477 笔  价差中位 2.323%  p90 11.687% 跑出带宽 13 笔（2.7%）

2026-08-26 那一期滞后是 **0 天**（想法成文当天就成交），114 笔里仍有 3 笔跑出
带宽——因为极低波动的标的上，σ×3 只有几个基点，比当天收盘价到成交价的差还小。
按现在的规模，**每周大约会有两笔仓的止损止盈挂在错误的一侧**。
...
**（甲）在成交时按成交价重算两条线。** `paper._fill` 现在把 `idea["stop_px"]`
和 `idea["take_lo"]` 原样抄到持仓上；改成用 `fill_px` 现算
（`fill_px × (1 ∓ Nσ)`），面板那句「进场即挂 σ×2 / σ×3」就**按构造为真**，
上面三类全部消失，也不再受成文与成交隔多久的影响。

**（乙）给风控带加一个绝对下限**（例如不低于往返成本的若干倍）。
量过之后**它解决不了主要问题**：812 笔里止盈带窄于 3 倍往返成本的只有 6 笔
（0.7%），窄于成本本身的 0 笔，带宽中位是 1586bp。窄带集中在 USFR 一只
（22.8bp）。所以（乙）只是（甲）之外的一层保险，不能替代它。

两者都是执行层的行为变更，会改变今后建仓的风控价位——**该由人决定**，
这个脚本不做，也不接进测试闸门：现在接进去会立刻变红，而红的那一刻会挡住
所有人的推送。
