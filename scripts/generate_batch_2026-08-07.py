...
def main() -> None:
    ...
    batch = {
        ...
        "macro_narrative":
            "当日 16 个词典主题里，能源供给与运输风险以 TIS 62.8 排第一，"
            "靠的是 N=92——新事实广度与因果深度都接近满分，这是全场最强的「有新东西」信号。"
            "它的 M=78 已进入「已有确认」，说明价格开始承认供给风险，"
            "但 C=48 仍属中性、距 52 周高点还有 9%，所以是「已确认但没拥挤」，"
            "本批次少数允许直接建仓的一类。"
            "紧随其后的盈利兑现与质量分化 D=100（248 条独立条目，全场最被讨论），"
            "但 C=80 也是全场最高——最被谈论的主题同时是最被交易过的主题，"
            "因此这条线连同国防（C=71）一律降级为等待回踩、仓位减半。"
            "真正最干净的一条是金属与战略补库：M=64 已确认，而 C=32 全场最低，"
            "只是 COPX 20 日已涨 18.7%，所以用回踩而不是追。"
            "AI 硬件的 C=13 是全场最不拥挤，但 TIS 只有 40.2（背景级），"
            "所以低仓位保留完整表达而不是加码。"
            "中国主题 M=14——价格几乎没反映，但语料也只有 8 条，属于「还没被交易」"
            "而不是「机会」，只留 1.5% 观察性敞口。"
