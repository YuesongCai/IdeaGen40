...
| | |
|---|---|
| 批次 | **每个交易日一批 40 条**，2026-07-27 起共 10 批 |
| 7/27 | 历史 PM pack，外部撰写，未经本系统拟合 |
| 7/28–8/06 | 规则引擎回填（`rules:v0.4`），每天独立打分 + 独立生成 |
| 8/07 | Claude 生成（`claude-code`） |
...
主口径是 **cohort（vintage）**：当天那 40 条自己成一个 1000 万美元的盘，
等权买入、持有到期限、单独计价。每天一盘，所以「哪天的想法好、哪天的差」可以直接比。
30 天后就是 30 行。

混合账户口径作为次要参考（页面上收在折叠里）：两个盘把每天的想法累加进同一个账户，
回答的是「这套流程对账户净值做了什么」——
**它回答不了「某一天的想法好不好」，因为一条混合曲线把所有 vintage 搅在一起了。**

- **守纪律组合** — 方法论仓位、进场区间限价、止损止盈、到期平仓，未成交的钱留现金计息
...
theme-candidates 语料里字典还没有词的宏观辩论；同时报字典覆盖率
theme-register   把判定过的新主题追加进注册表，盖上今天的日期
...
**7 — 主题不再是提前想全的一张固定表。**
最初 16 条主题冻结在代码里。冻结「关键问题与价格指标」是对的——否则 M 会自己验证自己。
但它顺带假设了主题的**集合**可以提前穷举，而这一点经不起数据检验：
**5,836 条语料里 2,688 条（46%）匹配不到任何主题。**
盲区里不是噪声——GLP-1 与医保准入、韩国科技股重估、人形机器人、
光收发器进口禁令、央行重启购金，当时都是有具名机构、有多天覆盖的真辩论，
但全部无法被打分，因此无法产出任何一条想法。最糟的是**固定字典报不出这个数**：
它把自己的盲区变成了看不见的东西。

v0.4.1 把字典拆成 **16 条种子（代码内，冻结）+ 只能追加的注册表**
（[`themes/registry.jsonl`](themes/registry.jsonl)）。每天从零匹配语料里机械挖候选——
词频 lift + 篇数/机构/天数门槛 + 极大化短语合并 + 共现聚类——
再由生成器判定哪些真的是主题（公司名和研报栏目名会漏进来，这是设计上的分工，
不是缺陷：机械挖掘负责找出「字典没有词的东西」，语义判断留给生成器）。

**唯一真正重要的规则是注册日不可回填**：`all_themes(as_of)` 只返回注册日 ≤ as_of 的主题。
没有这条，「主题发现」就是事后挑一个已经涨完的东西定义成主题、再为自己排序准确而自豪。
这条纪律在开发中已经抓到过一次真实泄漏：候选挖掘的同义词压制用了**全部**注册主题
而非 as-of 主题集，于是新主题一注册，历史重放就不再把它当候选了——
看起来像是早就发现了当时还没看到的东西。

字典覆盖率每天显示在看板的语料折叠区与 `ideagen sources`，当前约 **54%**。
这个数字下降就说明发现机制跟不上语料了。
...
ideagen/
  ...
  lexicon.py      16 条种子主题 + 注册表加载（as-of 闸门）+ 立场/因果深度/机构识别词表
  themes.py       主题发现：候选挖掘、注册校验（禁止回填）、休眠判定
...
prompts/
  ...
  theme_registration.md 主题注册契约：候选怎么判、字段要求、为什么注册日不可回填
themes/
  registry.jsonl       只能追加的发现主题注册表，每行一个，带注册日
docs/
...
三个视图，职责刻意分开，靠**点开**而不是往下滑：

| 视图 | 回答什么 | 日期轨 |
|---|---|---|
| **概览** | 跨全部天：日历、逐日柱状、想法层面能力、分档归因 | **没有**——它本来就是跨天汇总，放个日期选择器等于假装有选择 |
| **日报** | 某一天读到了什么、得出了什么判断 | 有 |
| **组合** | 某一天买了什么、现在怎么样 | 有 |

**日报只有两张表，都能点开。** 之前是「主题表 → Theme Map → 想法表 → 逐条卡片 →
来源大表」五段平铺，而 Theme Map 的叶子就是想法表里的 ticker，来源又全堆在页面最底部。
现在：

- **主题表** —— 一行一个主题。点开看：预注册关键结果、**D/A/B/N/M/C 各自是怎么算出来的**
  （条目数、机构数、三日覆盖率、意外程度来自哪个指标的哪次跳动、因果深度取了哪三条）、
  **该主题的原始图表**（图 + 智堡自己的解读 + 原图链接）、**该主题的证据逐条**
  （标题 + 立场 + 层级 + 机构 + 可复现检索式 + sha1 + 插图）、由此产生了哪几条想法。
- **想法表** —— 一行一个 ticker，**归类列直接写在行里**（宏观主题 / 传导主线 / 资产信号），
  Theme Map 的信息就在这儿，不必再单列一块。点开看：论点、最可能怎么错、三情景表、
  执行口径、成交与盯市、**这条引用的来源**（可点，带检索式）。
- 语料总量收进一个折叠——逐条证据已经挂在它支撑的主题下面了。

**组合的持仓也能点开**，直接看到产生这个仓位的那条想法的完整论点与来源。

原则是：**source 放在引用它的东西下面，不是页面最底部。**

单文件、深浅色自适应、表头可排序。宽表在自己的容器里横向滚动，页面本身不会横向溢出。
不确定的名词旁边是 `ⓘ`——浮层挂在 document body 上用 fixed 定位，
所以不会被表格的 `overflow-x` 裁掉（这是它最初的 bug）。

前端有一组不变量测试（`TestFrontEndInvariants`）盯着这些：浮层的层级归属、
单元格默认不换行、长文本三行截断、空值排在最后、两套主题都定义、
每个挂了 `ⓘ` 的词都在名词表里。写这些是因为开发中有两次字符串替换静默没命中，
悄悄删掉了规则而不是加上——这类失败必须让它报错。
同时输出 `web/report.json`，页面上的每个数字都能在里面找到。

**一个口径上的坑，已经在代码里防住了**：如果先生成了批次、之后又对同一天重新打分，
日报的叙述会和它下面的表格对不上——重新打分看到的语料更多，分数会变。
所以批次会把生成时的打分**快照**下来，日报优先显示快照；
而 `score` 命令在该日期已有 traded 批次时会直接跳过，除非加 `--force`。
...
每个过去日期的组合都会**盯到最后一个已收盘交易日**，不是冻结在建仓那天。
7/28 那一盘的 SMH 建仓价 529.71，现在显示的是 8/06 的 571.48；
明天打开就是 8/07 的价格。`ideagen mark` 会推进**全部**账本（两个混合盘 + 每个当日组合）。

仓位按期限自动离场：1 个月的到建仓日 +1 个月平仓，6 个月的到 +6 个月。
所以每一盘会慢慢空掉——7/27 那批最早的一批 1 个月期在 8/27 到期。
守纪律盘另有收盘止损与盘中止盈；当日组合刻意不设，它要的是纯粹的想法读数。
