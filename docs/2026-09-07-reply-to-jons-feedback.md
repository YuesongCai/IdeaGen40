...
2. **候选池一堆 LU/ISIN 基金，组合里没几个**：2026-09-02 的 79 只候选里 61 只是 Olive 货架基金，本机 `navs`/`shelf_navs` 净值表为空（211 只基金 `priceable=0`），它们在建仓时被「当日无价剔除」，AI 端到端等策略选的几乎全是这类，整本为空——筛选 C 一直在比较一堆根本无法盯市的候选。改法：候选池给每只候选打 `markable` 标记（表里带「无净值」标签，池子顶部写明只数），**筛选 C 只对可盯市的候选做选取**，剔除数写进运行记录（journal 步 `selectors:markable`）；候选池本身仍保存全部候选。要让基金真正回到组合，需要 Olive 净值腿提供历史净值，这是数据问题不是选取逻辑问题。下一期周跑起生效；已存的六期判决未重算。
