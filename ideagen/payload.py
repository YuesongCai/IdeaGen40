...
def _macro(con, dates) -> dict:
    """The macro layer block, shared with `/api/state`.

    One implementation in `macro.state_block`, called from both places. It was
    briefly only here — and `payload.build` feeds the legacy report, not
    `/api/state`, so the block reached a page nobody looks at while the panel's
    own document did not have it. Two copies would have drifted; one function
    with two callers cannot.
    """
    from . import macro
    d = dates[-1] if dates else config.today_hkt().isoformat()
    return macro.state_block(con, d)
